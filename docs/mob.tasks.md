...
- [ ] **M0.1** [C/D] Инициализация монорепо (workspaces), два Expo-приложения `client`/`driver`, общий `shared-core`.
- [ ] **M0.2** [C/D] TypeScript strict, ESLint + Prettier + правила границ FSD (eslint-plugin-boundaries), pre-commit хуки.
- [ ] **M0.3** [C/D] Настройка Expo Router (тонкие маршруты, делегирующие в слой `screens`), базовая навигация (стек + табы).
- [ ] **M0.4** [C/D] Redux Toolkit store: конфигурация, типизированные хуки (`useAppDispatch/useAppSelector`), redux-persist (не-секретное состояние).
- [ ] **M0.5** [C/D] RTK Query `baseApi`: `baseUrl` из env (`/api/v1`), `prepareHeaders` (Bearer), заголовок `Idempotency-Key` для мутаций (`§14.5`).
- [ ] **M0.6** [C/D] `shared/config`: окружения dev/staging/prod через `app.config.ts` + `EXPO_PUBLIC_*`.
- [ ] **M0.7** [C/D] Дизайн-система в `shared/ui`: токены (цвета/типографика/отступы), базовые компоненты (Button, Input, Text, Screen, Sheet), тема (в т.ч. учёт культурных особенностей региона).
- [ ] **M0.8** [C/D] i18n-каркас (i18next), ru по умолчанию, структура словарей, определение локали устройства. — *Req §24*
- [ ] **M0.9** [C/D] Обработка ошибок: global ErrorBoundary, Sentry (`@sentry/react-native`), единый формат ошибок API (`code/message/details`). — *Des §13*
- [ ] **M0.10** [C/D] `expo prebuild` + Dev Client; проверка запуска на реальном Android и iOS.
- [ ] **M0.11** [C/D] EAS Build профили (development/preview/production), EAS Update (OTA), базовый `eas.json`.
- [ ] **M0.12** [C/D] CI (GitHub Actions): lint → typecheck → тесты → EAS build (preview). — *Req §26*
...
- [ ] **M1.1** [C/D] Entity `session`: slice (tokens в SecureStore, статус авторизации, роль пользователя).
- [ ] **M1.2** [C/D] Экран ввода телефона → `POST /auth/otp/request`; маска/валидация номера (zod). — *API §14.1*
- [ ] **M1.3** [C/D] Экран ввода кода → `POST /auth/otp/verify`; таймер повторной отправки, лимиты/анти-перебор на UI. — *Req §20, Des §9*
- [ ] **M1.4** [C/D] Хранение JWT в `expo-secure-store`; автоподстановка Bearer в RTK Query.
- [ ] **M1.5** [C/D] Тихое обновление токена: перехват `401` в `baseQuery` → `POST /auth/refresh` (с ротацией) → повтор запроса; при неудаче — logout. — *Des §9, API §14.1*
- [ ] **M1.6** [C/D] Logout (`POST /auth/logout`), очистка SecureStore и redux-persist.
- [ ] **M1.7** [C] Онбординг после первой регистрации: запрос имени, опц. фото, **согласие на обработку ПДн (152-ФЗ)**. — *Req §8.1, §8.3, Des §9*
- [ ] **M1.8** [C/D] Guard-навигация: разделение стеков «гость / авторизован»; для водителя — учёт статуса верификации.
...
- [ ] **M2.1** [C] Entity `user`: RTK Query `GET/PATCH /me`, кэш профиля.
- [ ] **M2.2** [C] Экран профиля: фото (upload), имя, телефон, язык интерфейса. — *Req §8.3, §24*
- [ ] **M2.3** [C] Настройки уведомлений и приватности. — *Req §8.3*
- [ ] **M2.4** [C] Экстренные контакты: список + CRUD, лимит 5 (`GET/POST/DELETE /me/emergency-contacts`). — *Req §8.7, API §14.2*
- [ ] **M2.5** [C] Способы оплаты (список; привязка — вместе с Фазой M6). — *Req §8.3*
- [ ] **M2.6** [C] Переключение языка интерфейса на лету (i18n). — *Req §24*
...
- [ ] **M3.1** [C] Разрешения геолокации (foreground) через expo-location; UX-обработка отказа. — *Req §8.8*
- [ ] **M3.2** [C/D] Widget карты (`react-native-maps`) с абстракцией провайдера (Adapter, соответствует `design.md §4.3`): маркеры, центрирование, polyline.
- [ ] **M3.3** [C] Главный экран клиента: карта, «моё местоположение», поле поиска, кнопка заказа, меню профиля. — *Req §8.8*
- [ ] **M3.4** [C] Поиск адресов `GET /geo/search?q=` с debounce, учётом специфики адресации СК; результаты списком. — *Req §8.9, API §14.2*
- [ ] **M3.5** [C] Выбор точки на карте (pickup/dropoff), reverse-geocoding через провайдера. — *Req §8.10*
- [ ] **M3.6** [C] Entity `savedAddress`: любимые адреса (Дом/Работа/…), CRUD, без лимита (`GET/POST/DELETE /me/addresses`). — *Req §8.5, API §14.2*
- [ ] **M3.7** [C] Быстрый выбор сохранённого адреса как пункта назначения/отправления.
...
- [ ] **M4.1** [C] Entity `tariff`/`order`: типы, маппинг статусов конечного автомата (`§12.1`).
- [ ] **M4.2** [C] Расчёт стоимости/маршрута `POST /orders/estimate` (цена до заказа); карточка с ETA, дистанцией, ценой. — *Req §8.10, Des §7, API §14.2*
- [ ] **M4.3** [C] Выбор тарифа (список тарифов региона), способа оплаты, комментарий к заказу (`§8.11`). — *Req §8.10, §8.11*
- [ ] **M4.4** [C] Подтверждение и создание заказа `POST /orders` с `Idempotency-Key`. — *Req §8.10, §14.5*
- [ ] **M4.5** [C] Экран поиска водителя (`SEARCHING_DRIVER`): анимация, тайм-аут, обработка `NO_DRIVERS_FOUND` → предложение повторить. — *Req §12, §16 UC-1*
- [ ] **M4.6** [C] Отображение назначенного водителя и авто на карте (`DRIVER_ASSIGNED`/`EN_ROUTE`/`ARRIVED`): карточка водителя, ETA, кнопка звонка. — *Req §8.10*
- [ ] **M4.7** [C] Экраны стадий поездки (`IN_PROGRESS`, `COMPLETED`) на основе статусов заказа. — *Req §12*
- [ ] **M4.8** [C] Отмена заказа `POST /orders/{id}/cancel` на разрешённых стадиях; показ политики отмены/штрафа из тарифа. — *Req §8.12, API §14.2*
- [ ] **M4.9** [C] Блокировка нового заказа при активном (бизнес-правило `§9`), кроме заказа члену семьи (M10). — *Req §9*
- [ ] **M4.10** [C] Синхронизация статуса заказа: `GET /orders/{id}` (fallback-поллинг) до подключения WS (M5). — *API §14.2*
...
- [ ] **M5.1** [C/D] WebSocket-клиент: подключение с JWT, реконнект/бэкофф, подписка на каналы `client:{id}` / `order:{id}` (`§15.1`). — *Des §10*
- [ ] **M5.2** [C] Онлайн-обновление статуса заказа и позиции водителя на карте (без поллинга). — *Req §15.1*
- [ ] **M5.3** [C] Кнопка **SOS** во время поездки → `POST /orders/{id}/sos`; подтверждение, индикация активации. — *Req §8.7, §16 UC-3, API §14.2*
...
- [ ] **M5.5** [C/D] Обработка обрыва сети: индикатор офлайна, буферизация, восстановление подписок. — *Des §11 (graceful degradation)*
...
- [ ] **M6.2** [C] Оплата поездки при `COMPLETED`; обработка `FAILED_PAYMENT` и retry-статуса от сервера. — *Req §12, §22, Des §8.2*
- [ ] **M6.3** [C] Экран чека/квитанции после `CLOSED`. — *Req §8.13, §22*
- [ ] **M6.4** [C] История поездок `GET /orders/history` (cursor-пагинация `§14.5`): дата, маршрут, цена, водитель, статус, чек. — *Req §8.13, API §14.2*
- [ ] **M6.5** [C] Экран деталей поездки из истории.
- [ ] **M6.6** [C] Отзыв `POST /orders/{id}/review`: оценка 1–5, текст, теги (вежливость/чистота/вождение), жалоба. — *Req §8.14, API §14.2*
- [ ] **M6.7** [C] Бонусный баланс и промокоды (за feature-флагом региона). — *Req §8.3, Des §4.2*
...
- [ ] **M7.1** [D] Анкета водителя (ФИО, дата рождения, телефон, адрес, стаж, регион) → `POST /driver/register`; валидация zod. — *Req §8.2, API §14.3*
- [ ] **M7.2** [D] Загрузка документов (паспорт, ВУ, СТС, ОСАГО, фото авто/салона, селфи) `POST /driver/documents`; camera/gallery, сжатие, приватная загрузка. — *Req §8.2, Des §9, API §14.3*
- [ ] **M7.3** [D] Экран статуса верификации (`PENDING/REJECTED/APPROVED`), причины отклонения, повторная подача. — *Req §8.2, §12.3*
- [ ] **M7.4** [D] Блокировка выхода «на линию» и приёма заказов до одобрения. — *Req §8.2, §12.3*
...
- [ ] **M8.1** [D] Переключатель статуса `ONLINE/OFFLINE` → `PATCH /driver/status`. — *Req §12.3, API §14.3*
- [ ] **M8.2** [D] Фоновая передача геопозиции (expo-location background task) на сервер/WS для матчинга. — *Req §15.3, Des §6*
...
- [ ] **M10.1** [C/D] Push через `expo-notifications`: регистрация токена на сервере, каналы Android, разрешения iOS. — *Req §23*
- [ ] **M10.2** [C/D] Обработка push по ключевым событиям (`§23`): назначение, прибытие, завершение, оплата, верификация, SOS, промо; deep-link в нужный экран.
- [ ] **M10.3** [C/D] In-app уведомления и центр уведомлений. — *Req §23*
- [ ] **M10.4** [C] Семейный аккаунт: добавление/подтверждение членов семьи (`GET/POST/DELETE /me/family`), заказ и оплата поездки за члена семьи, совместное отслеживание. — *Req §8.6, §16 UC-4, API §14.2*
- [ ] **M10.5** [C] Избранные водители (влияет на матчинг). — *Req §15.3*
- [ ] **M10.6** [C/D] Полная локализация всех строк; i18n-ready для добавления языков. — *Req §24*
...
- [ ] **M11.6** [C/D] Безопасность клиента: токены только в SecureStore, отсутствие ПДн в логах, certificate pinning (опц.), обфускация. — *Req §20, Des §9*
