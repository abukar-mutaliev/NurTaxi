...
- [x] **W8.1** [R] Изоляция по `region_id`: все списки/действия ограничены своим регионом. — *Req §7.4, Des §9*
- [x] **W8.2** [R] Управление водителями региона: подтверждение документов, блокировка. — *Req §7.4*
- [x] **W8.3** [R] Управление тарифами своего региона (в пределах прав). — *Req §7.4*
- [x] **W8.4** [R] Просмотр региональной статистики. — *Req §7.4*
- [x] **W8.5** [R] Работа с обращениями пользователей региона. — *Req §7.4*
...
- [x] **W9.1** [A] `GET /admin/analytics`: агрегаты по региону/периоду. — *API §14.4*
- [x] **W9.2** [A] Дашборд KPI: время назначения водителя, доля успешных заказов/платежей, доступность, латентность (`§10.1`). — *Req §10.1, Des §13*
- [x] **W9.3** [A] Отчёты: заказы, водители, финансы; экспорт (CSV/Excel). — *Req §21*
- [x] **W9.4** [A] Графики (Recharts) с фильтрами по региону и датам; region-скоуп для [R]/[O].
...
- [x] **W10.1** [A] WebSocket-клиент: подключение с JWT, реконнект, подписки на служебные каналы. — *Des §10*
- [x] **W10.2** [O] Лайв-обновление списка/карты заказов и позиций водителей без перезагрузки. — *Req §15.1*
- [x] **W10.3** [O] **Приоритетные SOS-алёрты** оператору (событие `sos.activated`): всплывающее оповещение, координаты, данные водителя/авто, статус. — *Req §8.7, §16 UC-3, Des §9*
- [x] **W10.4** [A] Живые уведомления о ключевых событиях (`order.*`, `payment.failed`, `document.verified`). — *Des §10*
...
- [ ] **W11.1** [A] ~~Полная локализация интерфейса (ru), i18n-ready для новых языков.~~ *Пропущено по решению команды.*
- [x] **W11.2** [A] Security-hardening: CSP, отсутствие ПДн в логах, безопасное хранение токенов, проверка RBAC на всех маршрутах/действиях. — *Req §20, Des §9*
- [x] **W11.3** [A] Проверка изоляции регионов (нельзя увидеть/изменить чужой `region_id`). — *Req §7.4, §25*
- [x] **W11.4** [A] Unit/компонентные тесты (Vitest + RTL): RBAC, формы, таблицы, slices.
- [x] **W11.5** [A] E2E (Playwright): UC-5 (управление регионом), верификация водителя, операторский разбор заказа/возврат. — *Req §16, §25*
- [x] **W11.6** [A] Доступность (a11y), адаптивность, состояния загрузки/ошибок/пустых списков.
- [x] **W11.7** [A] Деплой на staging → production (Docker/Nginx), CI/CD с автотестами и сканированием. — *Req §26, Des §14*
