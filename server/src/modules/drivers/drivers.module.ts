...
import { RegistryDocumentAutoVerifier } from './verification/registry-document-auto-verifier';
import { DriverLocationService } from './location/driver-location.service';
import { RealtimeModule } from '../realtime/realtime.module';
import { TaxiRegistryModule } from '../taxi-registry/taxi-registry.module';
...
@Module({
  imports: [
    TypeOrmModule.forFeature([DriverProfile, DriverDocument, Vehicle, DriverTaxiPermit]),
    StorageModule,
    UsersModule,
    RegionsModule,
    forwardRef(() => RealtimeModule),
    forwardRef(() => PaymentsModule),
    forwardRef(() => TaxiRegistryModule),
  ],
  controllers: [DriversController],
  providers: [
    DriversService,
    DriverLocationService,
    { provide: DOCUMENT_AUTO_VERIFIER, useClass: RegistryDocumentAutoVerifier },
  ],
