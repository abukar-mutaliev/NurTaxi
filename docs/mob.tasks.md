...
| Клиент: остальные экраны (заказ, адреса, история, профиль, оплата) | маршруты-заглушки |
...
**Ближайшие шаги:** CI мобильного трека (M0.12) → сборка dev-клиента в EAS и запуск на
устройстве (M0.10, M0.11) → экраны заказа у клиента (M3.4–M4) и регистрации у водителя (M7).
...
> **Статус: готов слой данных.** Все эндпоинты и типы есть в `shared-core/entities`
> (`user`, `emergency-contact`, `payment`), переключение языка — в `shared/i18n`. Экраны не написаны.
...
- [~] **M2.4** [C] Экстренные контакты: список + CRUD, лимит 5 (`GET/POST/DELETE /me/emergency-contacts`). — *Req §8.7, API §14.2. Эндпоинты готовы, экрана нет*
- [ ] **M2.5** [C] Способы оплаты (список; привязка — вместе с Фазой M6). — *Req §8.3*
- [~] **M2.6** [C] Переключение языка интерфейса на лету (i18n). — *Req §24. `changeAppLanguage()` готов, экрана настроек нет*
...
> **Статус: основа готова.** Разрешения, карта с адаптером провайдера и главный экран
> работают; поиск адресов и работа с точками — следующий шаг.

- [x] **M3.1** [C] Разрешения геолокации (foreground) через expo-location; UX-обработка отказа. — *Req §8.8*
- [x] **M3.2** [C/D] Widget карты (`react-native-maps`) с абстракцией провайдера (Adapter, соответствует `design.md §4.3`): маркеры, центрирование, polyline.
- [~] **M3.3** [C] Главный экран клиента: карта, «моё местоположение», поле поиска, кнопка заказа, меню профиля. — *Req §8.8. Карта, центрирование, карточка активного заказа и вход в поиск готовы; сам поиск — заглушка*
- [~] **M3.4** [C] Поиск адресов `GET /geo/search?q=` с debounce, учётом специфики адресации СК; результаты списком. — *Req §8.9, API §14.2. Эндпоинт и `useDebouncedValue` готовы, экран — заглушка*
- [ ] **M3.5** [C] Выбор точки на карте (pickup/dropoff), reverse-geocoding через провайдера. — *Req §8.10*
- [~] **M3.6** [C] Entity `savedAddress`: любимые адреса (Дом/Работа/…), CRUD, без лимита (`GET/POST/DELETE /me/addresses`). — *Req §8.5, API §14.2. Эндпоинты готовы, экрана нет*
...
> **Статус: подготовлен фундамент.** Есть все эндпоинты, типы, конечный автомат статусов
> (`entities/order/model/order-status.ts`) и срез сквозного процесса
> (`apps/client/src/processes/order-flow`). Экраны сценария не написаны.

- [x] **M4.1** [C] Entity `tariff`/`order`: типы, маппинг статусов конечного автомата (`§12.1`).
- [~] **M4.2** [C] Расчёт стоимости/маршрута `POST /orders/estimate` (цена до заказа); карточка с ETA, дистанцией, ценой. — *Req §8.10, Des §7, API §14.2. Эндпоинт и форматтеры готовы, экрана нет*
- [ ] **M4.3** [C] Выбор тарифа (список тарифов региона), способа оплаты, комментарий к заказу (`§8.11`). — *Req §8.10, §8.11*
- [~] **M4.4** [C] Подтверждение и создание заказа `POST /orders` с `Idempotency-Key`. — *Req §8.10, §14.5. Мутация с автоматическим ключом идемпотентности готова, экрана нет*
...
- [~] **M4.8** [C] Отмена заказа `POST /orders/{id}/cancel` на разрешённых стадиях; показ политики отмены/штрафа из тарифа. — *Req §8.12, API §14.2. Мутация и предикат `canCancelOrder` готовы, экрана нет*
- [ ] **M4.9** [C] Блокировка нового заказа при активном (бизнес-правило `§9`), кроме заказа члену семьи (M10). — *Req §9*
- [~] **M4.10** [C] Синхронизация статуса заказа: `GET /orders/{id}` (fallback-поллинг) до подключения WS (M5). — *API §14.2. Запрос готов и используется на главном экране*
...
- [~] **M5.2** [C] Онлайн-обновление статуса заказа и позиции водителя на карте (без поллинга). — *Req §15.1. Кэш заказа и позиции обновляются по событиям WS; отрисовки движения на карте нет*
- [~] **M5.3** [C] Кнопка **SOS** во время поездки → `POST /orders/{id}/sos`; подтверждение, индикация активации. — *Req §8.7, §16 UC-3, API §14.2. Мутация и предикат `canActivateSos` готовы, кнопки нет*
...
- [~] **M5.5** [C/D] Обработка обрыва сети: индикатор офлайна, буферизация, восстановление подписок. — *Des §11 (graceful degradation). Реконнект и восстановление подписок готовы, индикатора офлайна в UI нет*
...
> **Статус: готов слой данных** (`entities/payment`, `entities/order`, `entities/promo`),
> экраны не написаны. История поездок открывается, но показывает заглушку.
...
- [~] **M6.4** [C] История поездок `GET /orders/history` (cursor-пагинация `§14.5`): дата, маршрут, цена, водитель, статус, чек. — *Req §8.13, API §14.2. Эндпоинт с курсорной пагинацией готов, экран показывает заглушку*
...
> **Статус: готов слой данных** (`entities/notification`, `entities/family`) и словари ru/en.

- [~] **M10.1** [C/D] Push через `expo-notifications`: регистрация токена на сервере, каналы Android, разрешения iOS. — *Req §23. Плагин, канал `orders` и эндпоинт регистрации токена готовы; запрос разрешений и отправка токена не подключены*
...
- [~] **M10.4** [C] Семейный аккаунт: добавление/подтверждение членов семьи (`GET/POST/DELETE /me/family`), заказ и оплата поездки за члена семьи, совместное отслеживание. — *Req §8.6, §16 UC-4, API §14.2. Эндпоинты готовы, экранов нет*
- [ ] **M10.5** [C] Избранные водители (влияет на матчинг). — *Req §15.3*
- [~] **M10.6** [C/D] Полная локализация всех строк; i18n-ready для добавления языков. — *Req §24. Словарь ru заполнен под весь план, en — частично; строки новых экранов добавляются по мере разработки*
